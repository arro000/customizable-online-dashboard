...
const PostItContent: React.FC<WidgetProps<PostItConfig>> = ({ id, config, onConfigChange }) => {
  const [history, setHistory] = useLocalStorage<string[]>(`postIt_${id}_history`, []);
...
const PostItOptions: React.FC<WidgetProps<PostItConfig>> = ({ id, config, onConfigChange }) => {
  const [history] = useLocalStorage<string[]>(`postIt_${id}_history`, []);
