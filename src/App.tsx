...
import MoodColorizer from "./components/widgets/MoodColorizer";
import PomodoroTimer from "./components/widgets/PomodoroTimer";
import Calculator from "./components/widgets/Calculator";
import NeuroNetVisualizer from "./components/widgets/NeuroNetVisualizer";
import MoodMusicMixer from "./components/widgets/MoodMusicMixer";
import ConfigurableIframeCard from "./components/widgets/ConfigurableIframeCard";
import InteractiveMindMap from "./components/widgets/InteractiveMindMap";
import DynamicDashboard from "./components/widgets/DynamicDashboard";
import AdvancedDataVisualizer from "./components/widgets/AdvancedDataVisualizer";
import { CardData } from "./components/widgets/Card";
...
const App: React.FC = () => {
  ...
  const [shouldResetPosition, setShouldResetPosition] =
  ...
  const updateCard = (id: string, newData: { title: string; url: string }) => {
    setCards((prevCards) =>
      prevCards.map((card) => (card.id === id ? { ...card, ...newData } : card))
    );
  };
  ...
  const onDragEnd = (id: string, newIndex: number) => {
    setCards((prevCards) => {
      ...
      if (draggedCardIndex === -1 || draggedCardIndex === newIndex)
        return prevCards;
    ...
    if (url) {
      try {
        ...
        toast({
          title: "Errore",
          description:
            "Non è stato possibile aggiungere la card. Riprova più tardi.",
          status: "error",
          duration: 3000,
          isClosable: true,
        });
